...
export interface Bf {
    ...
    scrapeText(url: string, opts?: Omit<ScrapePayload, "url">): Promise<{
        text: string;
        document: ScrapeResult;
    }>;
    scrapeMarkdown(url: string, opts?: Omit<ScrapePayload, "url">): Promise<{
        markdown: string;
        document: ScrapeResult;
    }>;
    fetchText(url: string, opts?: Omit<FetchPayload, "url">): Promise<{
        text: string;
        status?: number;
        final_url?: string;
        title?: string;
    }>;
    fetchJson(url: string, opts?: Omit<FetchPayload, "url">): Promise<{
        json: unknown;
        status?: number;
        final_url?: string;
    }>;
    screenshot(url: string, opts?: Omit<FetchPayload, "url" | "screenshot">): Promise<{
        png_b64: string;
        status?: number;
    }>;
}
