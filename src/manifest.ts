...
export function validateManifest(raw: unknown): ToolManifest {
  return manifestSchema.parse(raw) as ToolManifest;
}
 ...
 */
export function checkOutputMatches(
  output: unknown,
  ...
  for (const [path, expected] of Object.entries(outputMatches)) {
    ...
    if (
      ...
      "minLength" in (expected as object)
    ) {
      const min = (expected as { minLength: number }).minLength;
