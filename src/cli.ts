...
import type { Bf, ToolManifest, ToolRun } from "./types.js";
...
function bundle(manifest: ToolManifest): string {
  ...
  if (result.errors.length) {
    fail(
      `bundle failed (tools must be pure JS — no node builtins):\n` +
...
function loadRun(code: string): ToolRun {
  const mod = new Function(`${code}; return __tool;`)() as { default?: ToolRun };
...
async function main() {
  ...
  switch (command) {
    case "validate": {
      ...
      if (manifest.outputSchema) outputIoSchema.parse(manifest.outputSchema);
      const code = bundle(manifest);
      loadRun(code);
      console.log(
        `ok: ${manifest.name}@${manifest.version} (bundle ${Buffer.byteLength(code)} bytes, ` +
    ...
    case "run": {
      ...
      if (exampleName) {
        ...
        if (!example) fail(`no example named "${exampleName}"`);
        input = example.input;
      } else if (inputRaw) {
        try {
    ...
    case "test": {
      ...
      if (failed) fail(`${failed}/${manifest.examples.length} examples failed`);
